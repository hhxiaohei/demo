...
class AdminsController extends BaseController {
    ...
    public function store(CreateAdminRequest $request, Registrar $registrar)
    {
        ...
        $registrar->create($request->only(['name', 'mobile', 'email','password']));
    ...
    public function update($id, UpdateAdminRequest $request)
    {
        ...
        $count = Admin::whereMobile($mobile)->where('id', '!=', $id)->count();
        if ($count > 0) {
            return $this->redirectWithError('手机号不能重复');
        }
        $password = $request->get('password');
        $data = $request->only(['name', 'email', 'mobile','openid']);
        if ($password) {
            $data['password'] = Hash::make($password);
        }
        $res= Admin::findOrFail($id)->update($data);
        return redirect()->route('admin.admins.index');
    }
